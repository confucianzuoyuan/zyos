...
struct heap
{
    pagetable_t          *pt;           // 拥有堆的页表
    void                 *vaddr;        // 堆开始的地址
    uint64_t              pages;        // 当前分配到堆的页
    uint64_t              maxpages;     // 堆使用的最大的页数量
    struct fblock_header *first_fblock; // 堆中第一个空闲块
    uint64_t              reserved;     // 对齐到16字节的整数倍
};
